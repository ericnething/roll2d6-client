        ...
        ChangesReceived changes ->
            case decodeChanges model.gameType changes of
                ...
                Err err ->
                    let
                        _ =
                            Debug.log "Changes Received Error" err
                    in
                    ( model, Cmd.none )
...
debouncedWriteGameToPouchDB { ref, title, gameType, sheetsOrdering, sheetPermissions } =
    toCmd
...
writeGameToPouchDB { ref, title, gameType, sheetsOrdering, sheetPermissions } =
    toCmd
        (WriteGameToPouchDB ref
            "game"
            { title = title
